...
/**
 * @title DeployVarianceSwap
 * @notice Deploys VarianceSwap deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployVarianceSwap is Script {
    ...
    uint256 public _strikePerSecond;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint256 public _capPerSecond;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint64 public _minTerm;

    /// @notice Set before deploying: this is part of what the pool is, and it cannot change afterwards.
    uint64 public _maxTerm;
    ...
    function run() external {
        ...
        bytes memory creationCode = type(VarianceSwapHook).creationCode;
        bytes memory constructorArgs = abi.encode(manager, _strikePerSecond, _capPerSecond, _minTerm, _maxTerm, shareName, shareSymbol);
        ...
        console2.log("VarianceSwap ", predicted);
        ...
        vm.startBroadcast();
        VarianceSwapHook hook = new VarianceSwapHook{salt: salt}(manager, _strikePerSecond, _capPerSecond, _minTerm, _maxTerm, shareName, shareSymbol);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("VarianceSwap")),

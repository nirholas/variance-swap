# Integrating VarianceSwap

## From Solidity

```solidity
import {VarianceSwapHook} from "variance-swap/src/hooks/VarianceSwapHook.sol";
...
const hook = getHook("variance-swap");
const address = hookAddress("variance-swap", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "VarianceSwap"
...
More at https://variance-swap.pages.dev.
